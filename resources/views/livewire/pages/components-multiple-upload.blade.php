<div>
<section class="section">
          <x-stisla.section-header title="Multiple Upload">
        <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
              <div class="breadcrumb-item"><a href="#">Components</a></div>
              <div class="breadcrumb-item">Multiple Upload</div>
            </div>
    </x-stisla.section-header>

    <div class="section-body">
            <h2 class="section-title">Multiple Upload</h2>
            <p class="section-lead">
              We use 'Dropzone.js' made by @Dropzone. You can check the full documentation <a href="http://www.dropzonejs.com/">here</a>.
            </p>

            <div class="row">
              <div class="col-12">
                <x-stisla.card title="Multiple Upload">
<form action="#" class="dropzone" id="mydropzone">
                      <div class="fallback">
                        <input name="file" type="file" multiple />
                      </div>
                    </form>
</x-stisla.card>
              </div>
            </div>
          </div>
        </section>

@push('general-css')
<link rel="stylesheet" href="/assets/modules/dropzone/dist/min/dropzone.min.css">
@endpush

@push('js-libraries')
<script src="/assets/modules/dropzone/dist/min/dropzone.min.js"></script>
@endpush

@push('scripts')
<script src="/assets/js/page/components-multiple-upload.js"></script>
@endpush
</div>